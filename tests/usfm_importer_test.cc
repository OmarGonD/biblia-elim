...
int main(int argc, char **argv)
{
	...
	g_assert_cmpuint(stats.footnotesImported, ==, 1);
	g_assert_cmpuint(stats.crossReferencesImported, ==, 1);
	...
	g_assert_true(backend.moduleCapabilities("usfm-fixture").footnotes);
	g_assert_true(backend.moduleCapabilities("usfm-fixture").crossrefs);
	BibleKeyInfo key;
	...
	BibleKeyInfo richKey; g_assert_true(backend.resolveKey("usfm-fixture", "John 3:16", richKey));
	BibleVerseContent rich = backend.getVerseContent("usfm-fixture", richKey.reference);
	g_assert_cmpuint(rich.footnotes.size(), ==, 1);
	g_assert_cmpuint(rich.crossReferences.size(), ==, 0);
	BibleKeyInfo xKey; g_assert_true(backend.resolveKey("usfm-fixture", "John 3:17", xKey));
	BibleVerseContent cross = backend.getVerseContent("usfm-fixture", xKey.reference);
	g_assert_cmpuint(cross.crossReferences.size(), ==, 1);
	g_assert_cmpuint(cross.crossReferences[0].references.size(), ==, 2);
