...
class BackEnd : public BibleBackend
{
	...
	/* BibleBackend API.  These methods are implemented by translating to the
	 * existing long-lived SWORD manager; no second manager is created. */
	std::vector<BibleModuleInfo> listModules() const override;
	bool hasModule(const std::string &module_id) const override;
	std::string getText(const std::string &module_id,
	...
	std::vector<BibleVerse> getChapter(const std::string &module_id,
					    ...
					    bool rendered) override;
	BibleVerseContent getVerseContent(
	...
	bool getCurrentEntryFootnote(const std::string &module_id,
				     const std::string &id,
				     BibleFootnote &footnote) override;
	std::vector<std::string> getCurrentEntryCrossReferences(
	...
	std::string setVerse(const std::string &module_id,
					 const std::string &key,
					 int verse) override;
	std::string setBook(const std::string &module_id,
					const std::string &key,
					int testament,
					int book) override;
	std::vector<std::string> bookNames(const std::string &module_id,
							  int testament) const override;
	std::vector<BibleSearchResult> search(
		const std::string &module_id,
		const BibleSearchQuery &query) override;
	void setGlobalOption(const std::string &option, bool enabled) override;
	void setOption(BibleOption option, bool enabled) override;
	void configureRendering(const std::string &module_id,
				bool morphology_first,
				bool render_note_numbers) override;
