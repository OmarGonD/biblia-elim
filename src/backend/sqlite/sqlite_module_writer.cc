...
bool SqliteModuleWriter::write(const SqliteModuleMetadata &m,
    ...
    bool hs=false,hf=false,hx=false; for (const auto &v:verses) { hf|=!v.footnotes.empty(); hx|=!v.crossReferences.empty(); for(const auto&w:v.words) hs|=!w.strongs.empty(); }
    bool hh=false; for(const auto&v:verses) hh |= !v.headings.empty();
    std::map<std::string,std::string> md={{"schema_version","1"},{"module_id",m.moduleId},{"name",m.name},{"language",m.language},{"module_type","bible"},{"versification",m.versification},{"source_format",m.sourceFormat},{"feature.verses","true"},{"feature.search","true"},{"feature.strong",hs?"true":"false"},{"feature.morphology","false"},{"feature.headings",hh?"true":"false"},{"feature.footnotes",hf?"true":"false"},{"feature.crossrefs",hx?"true":"false"},{"feature.dictionary","false"}};
