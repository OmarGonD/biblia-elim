...
def audit(xml_path, *, volume, witness, book="Ps", limit=None,
    ...
    if visible_2_path.exists():
        report["verse_segmentation_audit"][
            "projected_form_a_visible_2_discriminator"] = json.loads(
                visible_2_path.read_text(encoding="utf-8"))["audit_summary"]
    visible_2_dry_run_path = Path(ROOT) / (
        "data/torresamat1835/projected_form_a_visible_2_dry_run.json")
    if visible_2_dry_run_path.exists():
        report["verse_segmentation_audit"][
    ...
    refined_scope_dry_run_path = Path(ROOT) / (
        "data/torresamat1835/projected_form_a_refined_scope_dry_run.json")
    if refined_scope_dry_run_path.exists():
        report["verse_segmentation_audit"][
